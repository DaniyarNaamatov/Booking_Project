...
from Booking.task import send_email_message
...
class RegisterSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        def create(self, validated_data):
            ...
            send_email_message.delay(code, user.email, status='register')
            return user
...
class LoginSerializer(serializers.Serializer):
    ...
    def validate_email(self, email):
        if not User.objects.filter(email=email).exitst():
...
class ForgotSerializer(serializers.Serializer):
    ...
    def send_code(self):
        ...
        user.save()
        send_email_message.delay(email=user.email, code=user.activation_code, status='reset_password')
